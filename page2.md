...
<div class="message-box">
  <div id="auth-ui" style="margin-bottom: 20px; padding: 15px; background: rgba(20, 20, 20, 0.6); border-radius: 8px; border: 1px solid #00f0ff; text-align: center;">
    ...
    <div id="login-options" style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; margin-bottom: 15px;">
      <a href="/page12" class="submit-btn" style="display: inline-block; background: transparent; color: #ffd700; border: 1px solid #ffd700; padding: 10px 20px; text-decoration: none; border-radius: 4px; font-family: 'Cormorant Garamond', serif; font-weight: bold; font-size: 1.1em; transition: all 0.3s ease;">Sign Up</a>
      <a href="/page13" class="submit-btn" style="display: inline-block; background: transparent; color: #ffd700; border: 1px solid #ffd700; padding: 10px 20px; text-decoration: none; border-radius: 4px; font-family: 'Cormorant Garamond', serif; font-weight: bold; font-size: 1.1em; transition: all 0.3s ease;">Sign In</a>
    </div>
  ...
  <form id="messageForm" style="display:none; margin-bottom: 30px;">
    ...
    <button type="submit" id="submitBtn" class="submit-btn" style="margin-top:10px; background: transparent; color: #ffd700; border: 1px solid #ffd700; padding: 10px 25px; cursor: pointer; border-radius: 4px; font-family: 'Cormorant Garamond', serif; font-weight: bold; font-size: 1.1em;">Post Message</button>
  </form>
...
<script>
  ...
  function addMessageToUI(msg) {
    ...
    const deleteBtn = (isOwner || isAdmin) 
      ? `<button onclick="deleteMsg('${msg.id}')" style="position:absolute; top:10px; right:10px; color:#ff4500; border:none; background:none; cursor:pointer; font-weight:bold; font-family: 'Cormorant Garamond', serif;">Delete</button>` 
      : '';

        div.innerHTML = `
      ${editBtn}
      ${deleteBtn}
      <strong style="color:#ff944d; font-family: 'Cormorant Garamond', serif;">${msg.username || 'Anonymous'}</strong> 
      <small class="timestamp" style="margin-left:8px; font-family: 'Cormorant Garamond', serif;">${new Date(msg.created_at).toLocaleString()}</small>
      <p style="margin: 10px 0 0 0; color:#ffd700; line-height:1.5; font-family: 'Cormorant Garamond', serif;">
...
<script>
  ...
  async function fetchAllVisitors() {
    ...
    if (error) {
      container.innerHTML = '<p style="color:red; text-align:center; padding:10px; font-family: \'Cormor
